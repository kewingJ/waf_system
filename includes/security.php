...
function timeAgo($datefrom,$dateto=-1)
	{
		...
		if($datefrom<=0) { return "Hace mucho tiempo"; }
		...
		$res = ($datediff==1) ? "hace $datediff meses" : "hace $datediff
		meses";
		break;
		
		case "y":
		$datediff = floor($difference / 60 / 60 / 24 / 365);
		$res = ($datediff==1) ? "hace $datediff años" : "hace $datediff
		años";
		break;
		
		case "d":
		$datediff = floor($difference / 60 / 60 / 24);
		$res = ($datediff==1) ? "hace $datediff días" : "hace $datediff
		días";
		break;
		
		case "ww":
		$datediff = floor($difference / 60 / 60 / 24 / 7);
		$res = ($datediff==1) ? "hace $datediff semanas" : 
        "hace $datediff semanas";
		break;
		
		case "h":
		$datediff = floor($difference / 60 / 60);
		$res = ($datediff==1) ? "hace $datediff horas" : 
        "hace $datediff horas";
		break;
		
		case "n":
		$datediff = floor($difference / 60);
		$res = ($datediff==1) ? "hace $datediff minutos" :
		"hace $datediff minutos";
		break;
		
		case "s":
		$datediff = $difference;
		$res = ($datediff==1) ? "hace $datediff segundos" :
		"hace $datediff segundos";
		break;
		}
		return $res;
		}
