<?php
    ...
    $cronLock = waf_acquire_cron_lock('ajax_convertion_dinamico');
	
	error_reporting(E_ALL);
    ini_set('display_errors', '1');
	
	$i = 1;
	$fp = fopen("../uploads/waf.log", "r");
    if ($fp === false) {
        exit('No se pudo abrir uploads/waf.log' . PHP_EOL);
    }
    $hayCambios = false;
	...
	fclose($fp);
    //eliminar archivo
    if (is_file('../uploads/waf.log')) {
        unlink('../uploads/waf.log');
    }
